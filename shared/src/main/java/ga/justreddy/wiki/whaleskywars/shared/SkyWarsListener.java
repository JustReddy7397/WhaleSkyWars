 ...
 * @author JustReddy
 */
public abstract class SkyWarsListener {
    ...
    protected  <T> void handleAckResponse(CompletableFuture<T> future, Object[] args, PacketType type) {
        if (future.isDone()) {
            return;
        }
        ...
        try {
            ...
            final JsonParseException jsonParseException = new JsonParseException("Failed to parse JSON for " + type.getType().getTypeName(), ex);
            future.completeExceptionally(jsonParseException);
